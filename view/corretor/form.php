...
?>
<h2 class="text-xl font-semibold text-gray-900 mb-6"><?= $editando ? 'Editar Corretor' : 'Novo Corretor' ?></h2>

<form method="post" action="index.php?entidade=corretor&acao=salvar" class="bg-white border border-gray-200 rounded-lg p-7 max-w-lg flex flex-col gap-5">

    <?php if ($editando): ?>
        <!-- ID necessário para distinguir UPDATE de INSERT no controller -->
        <input type="hidden" name="id" value="<?= $corretor->getId() ?>">
    <?php endif; ?>
    ...
    <label class="flex flex-col gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Nome
        <input type="text" name="nome" required class="w-full px-3 py-2 text-sm text-gray-900 bg-white border border-gray-300 rounded-md outline-none focus:border-gray-500 focus:ring-2 focus:ring-gray-200 transition"
               ...
               value="<?= htmlspecialchars($corretor?->getNome() ?? '') ?>">
    </label>

    <label class="flex flex-col gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wide">CRECI
        <!-- Número de registro obrigatório por lei para corretores de imóveis -->
        <input type="text" name="creci" required class="w-full px-3 py-2 text-sm text-gray-900 bg-white border border-gray-300 rounded-md outline-none focus:border-gray-500 focus:ring-2 focus:ring-gray-200 transition"
    ...
    <label class="flex flex-col gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Telefone
        <input type="text" name="telefone" class="w-full px-3 py-2 text-sm text-gray-900 bg-white border border-gray-300 rounded-md outline-none focus:border-gray-500 focus:ring-2 focus:ring-gray-200 transition"
               ...
               value="<?= htmlspecialchars($corretor?->getTelefone() ?? '') ?>">
    </label>
    ...
    <label class="flex flex-col gap-1.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Email
        <input type="email" name="email" class="w-full px-3 py-2 text-sm text-gray-900 bg-white border border-gray-300 rounded-md outline-none focus:border-gray-500 focus:ring-2 focus:ring-gray-200 transition"
               ...
               value="<?= htmlspecialchars($corretor?->getEmail() ?? '') ?>">
    </label>
    ...
    <a href="index.php?entidade=corretor&acao=listar" class="self-start text-xs text-gray-400 hover:text-gray-700 hover:underline transition">Cancelar</a>
</form>
